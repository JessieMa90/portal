    ...
    function getRoleActions() {
        ...
        if (!cookieVal) {
            ...
            if (cookieVal == '*') {
                ...
                role_actions = cookieVal.split('|');
            }
        }
        return role_actions;
    }
    var role_actions = getRoleActions();
    var attrName = 'data-action';
    var replaceAttrName = 'data-action-replace';
    var replaceUrlAttrName = 'data-action-replace-url';
    $('[' + attrName + ']').livequery(function (index, elm) {
        ...
        var action = $elm.attr(attrName);
        if (role_actions.indexOf(action) == -1) {
            var replacement = $elm.attr(replaceAttrName);
            var url = $elm.attr(replaceUrlAttrName);
            if (replacement) {
            ...
            if ($elm.hasClass('navitem')) {
                $(document).trigger('change.navi');
            }
            
        }
        else {
            $elm.removeAttr(attrName).removeAttr(replaceAttrName).removeAttr(replaceUrlAttrName);
        }
    });
